...
void print_benchmark_results(void) {
  ...
  printf("[task] preproc: %f, %f, %f\r\n", (double)min, (double)mean,
         (double)max);
  benchmark_get_result_between_ms(TASK_QUANTIZE_BEGIN,
  ...
  printf("[task] full runtime (not running net): %f, %f, %f\r\n", (double)min,
         (double)mean, (double)max);
...
void ai_task(void *pvParameters) {
  ...
  //   ai_model_init();
  //   ai_get_input_quant_details(&NET_INPUT_SCALE, &NET_INPUT_ZERO_POINT);
  preprocess_init_f32();
  preprocess_init_q31();
  preprocess_init_q15();
  ...
  while (1) {
    ...
    if (xSemaphoreTake(wave_ready_semaphore, portMAX_DELAY) == pdTRUE) {
      ...
      preprocess_quantize_mfcc_f32(mfcc_f32, mfcc, NET_INPUT_ZERO_POINT,
                                   NET_INPUT_SCALE);
#elif IMPLEMENTATION_OPTION == 1
      preprocess_calculate_f32(waveform, mfcc_f32);
      benchmark_set_point(TASK_QUANTIZE_BEGIN);
      preprocess_quantize_mfcc_f32_naive(mfcc_f32, mfcc, NET_INPUT_ZERO_POINT,
                                         NET_INPUT_SCALE);
#elif IMPLEMENTATION_OPTION == 2
      preprocess_calculate_q31(waveform, mfcc_q31);
      benchmark_set_point(TASK_QUANTIZE_BEGIN);
      preprocess_quantize_mfcc_q31_naive(mfcc_q31, mfcc, NET_INPUT_ZERO_POINT,
                                         NET_INPUT_SCALE);
#elif IMPLEMENTATION_OPTION == 3
      preprocess_calculate_q15(waveform, mfcc_q15);
      benchmark_set_point(TASK_QUANTIZE_BEGIN);
      preprocess_quantize_mfcc_q15_naive(mfcc_q15, mfcc, NET_INPUT_ZERO_POINT,
                                         NET_INPUT_SCALE);
#endif
